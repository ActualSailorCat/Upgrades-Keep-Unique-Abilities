...
AND NOT EXISTS (SELECT CivUniqueUnitType FROM UnitReplaces WHERE CivUniqueUnitType = 'UNIT_ARABIAN_CAMEL_ARCHER');
...
AND NOT EXISTS (SELECT CivUniqueUnitType FROM UnitReplaces WHERE CivUniqueUnitType = 'UNIT_MONGOLIAN_HUI_HUI_PAO');
...
-- ///////////////////////////////////////////////////////
-- // EnigmaConundrum
-- Supported: Hawaiian Kaimiloa & Koa
-- UNIT_ENIGMA_KOA
UPDATE RequirementSets SET RequirementSetType = 'REQUIREMENTSET_TEST_ANY' WHERE RequirementSetId = 'ENIGMA_KOA_EXPERIENCE_REQSET';
INSERT OR REPLACE INTO RequirementSetRequirements (RequirementSetId, RequirementId)
SELECT 'ENIGMA_KOA_EXPERIENCE_REQSET', 'SAILOR_PUA_ADJACENT_'||UnitType
...
(PrereqTech IN (SELECT TechnologyType FROM Technologies WHERE EraType IN (SELECT EraType FROM Eras WHERE ROWID > 0)) OR
PrereqCivic IN (SELECT CivicType FROM Civics WHERE EraType IN (SELECT EraType FROM Eras WHERE ROWID > 0))))
AND EXISTS (SELECT UnitType FROM Units WHERE UnitType = 'UNIT_ENIGMA_KOA');
...
-- // JFD
-- Supported: Soviet Pekhota, Soviet T-34

-- // Leugi
-- Supported: Brazilian Pracinha, Mexican Chinaco
-- UNIT_LEU_CHINACO
INSERT OR REPLACE INTO UnitAbilityModifiers (UnitAbilityType, ModifierId)
SELECT 'ABILITY_LEU_CUERUDO_DOUBLEMOVE', '-1_SAILOR_PUA_MOVEMENT'
WHERE EXISTS (SELECT UnitType FROM Units WHERE UnitType = 'UNIT_LEU_CHINACO')
AND NOT EXISTS (SELECT CivUniqueUnitType FROM UnitReplaces WHERE CivUniqueUnitType = 'UNIT_LEU_CHINACO');

INSERT OR REPLACE INTO UnitAbilityModifiers (UnitAbilityType, ModifierId)
SELECT 'ABILITY_LEU_CUERUDO_DOUBLEMOVE', '-2_SAILOR_PUA_STR'
WHERE EXISTS (SELECT UnitType FROM Units WHERE UnitType = 'UNIT_LEU_CHINACO')
AND NOT EXISTS (SELECT CivUniqueUnitType FROM UnitReplaces WHERE CivUniqueUnitType = 'UNIT_LEU_CHINACO');

-- // Mastodonmk2
-- Hyperdimension Neptunia: Planeptune Civilization
-- Supported: Nepgeardam, Dogoo Knight, Umio Warrior
-- UNIT_BWH_PLANEPTUNE_PLUTIADOLL
UPDATE RequirementSets SET RequirementSetType = 'REQUIREMENTSET_TEST_ANY' WHERE RequirementSetId = 'BWH_DOLL_PLOT_IS_DOLL_REQUIREMENTS';
INSERT OR REPLACE INTO RequirementSetRequirements (RequirementSetId, RequirementId)
SELECT 'BWH_DOLL_PLOT_IS_DOLL_REQUIREMENTS', 'SAILOR_PUA_ADJACENT_'||UnitType
FROM Units WHERE UnitType IN (SELECT UnitType FROM Units WHERE PromotionClass = 'PROMOTION_CLASS_MELEE' AND (PrereqTech IS NOT NULL OR PrereqCivic IS NOT NULL) AND 
(PrereqTech IN (SELECT TechnologyType FROM Technologies WHERE EraType IN (SELECT EraType FROM Eras WHERE ROWID > 0)) OR
PrereqCivic IN (SELECT CivicType FROM Civics WHERE EraType IN (SELECT EraType FROM Eras WHERE ROWID > 0))))
AND EXISTS (SELECT UnitType FROM Units WHERE UnitType = 'UNIT_BWH_PLANEPTUNE_PLUTIADOLL');
...
-- // Suk
-- Supported: Siamese Chang Suek, Gaul Oathsworn, Ethiopian Shotelai, Burmese Myinsi, Tibetan Rta Pa
-- UNIT_SUK_TIBET_RTA_PA
INSERT OR REPLACE INTO UnitAbilityModifiers (UnitAbilityType, ModifierId)
SELECT 'ABILITY_SUK_TIBET_RTA_PA', '-1_SAILOR_PUA_RANGE'
WHERE EXISTS (SELECT UnitType FROM Units WHERE UnitType = 'UNIT_SUK_TIBET_RTA_PA')
AND NOT EXISTS (SELECT CivUniqueUnitType FROM UnitReplaces WHERE CivUniqueUnitType = 'UNIT_SUK_TIBET_RTA_PA');

INSERT OR REPLACE INTO UnitAbilityModifiers (UnitAbilityType, ModifierId)
SELECT 'ABILITY_SUK_TIBET_RTA_PA', '2_SAILOR_PUA_MOVEMENT'
WHERE EXISTS (SELECT UnitType FROM Units WHERE UnitType = 'UNIT_SUK_TIBET_RTA_PA')
AND NOT EXISTS (SELECT CivUniqueUnitType FROM UnitReplaces WHERE CivUniqueUnitType = 'UNIT_SUK_TIBET_RTA_PA');

INSERT OR REPLACE INTO UnitAbilityModifiers (UnitAbilityType, ModifierId)
SELECT 'ABILITY_SUK_TIBET_RTA_PA', '5_SAILOR_PUA_STR'
WHERE EXISTS (SELECT UnitType FROM Units WHERE UnitType = 'UNIT_SUK_TIBET_RTA_PA')
AND NOT EXISTS (SELECT CivUniqueUnitType FROM UnitReplaces WHERE CivUniqueUnitType = 'UNIT_SUK_TIBET_RTA_PA');
